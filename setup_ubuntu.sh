...
sudo apt install -y curl net-tools python3-pip tmux tree vim-gnome zsh
pip3 install psutil trash-cli powerline-status

# Optional:
# sudo apt install -y build-essential git ibus-pinyin cmake exuberant-ctags clang-format
...
# Recommend: rsync -vcharz --progress ~/.vim $SERVER:
